...
impl Crypto {
    pub fn encrypt_block(&self, key: &[u8], index: u64, plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    pub fn decrypt_block(&self, key: &[u8], index: u64, blob: &[u8]) -> Result<Vec<u8>> {
        if blob.len() < NONCE_LEN + 16 {
            bail!("block ciphertext too short");
        }
        let (nonce, ct) = blob.split_at(NONCE_LEN);
        let nonce = XNonce::try_from(nonce).map_err(|_| anyhow::anyhow!("invalid block nonce"))?;
        let cipher = build_cipher(key)?;
        cipher
            .decrypt(
                &nonce,
            ...
            .context("block decryption failed (corrupted, tampered or reordered data)")
    }
}
